...
pub struct PersonsServiceApiClient {
    configuration: Rc<configuration::Configuration>,
}

impl PersonsServiceApiClient {
    pub fn new(configuration: Rc<configuration::Configuration>) -> PersonsServiceApiClient {
        PersonsServiceApiClient {
            configuration,
        }
    }
}
...
impl PersonsServiceApiClient {
    ...
    pub async fn get_persons_by_name(&self, name: &str, user_id: Option<&str>) -> Result<Box<crate::models::BaseItemDto>, Error> {
        let configuration: &configuration::Configuration = self.configuration.borrow();
        let req_client = &configuration.client;

        let uri_str = format!("{}/Persons/{Name}", configuration.base_path, Name=crate::apis::urlencode(name));
        let mut req_builder = req_client.request(reqwest::Method::GET, uri_str.as_str());
