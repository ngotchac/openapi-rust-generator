...
pub struct GenresServiceApiClient {
    configuration: Rc<configuration::Configuration>,
}

impl GenresServiceApiClient {
    pub fn new(configuration: Rc<configuration::Configuration>) -> GenresServiceApiClient {
        GenresServiceApiClient {
            configuration,
        }
    }
}
...
impl GenresServiceApiClient {
    ...
    pub async fn get_genres_by_name(&self, name: &str, user_id: Option<&str>) -> Result<Box<crate::models::BaseItemDto>, Error> {
        let configuration: &configuration::Configuration = self.configuration.borrow();
        let req_client = &configuration.client;

        let uri_str = format!("{}/Genres/{Name}", configuration.base_path, Name=crate::apis::urlencode(name));
        let mut req_builder = req_client.request(reqwest::Method::GET, uri_str.as_str());
